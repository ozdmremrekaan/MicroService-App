...
namespace Mango.Web.Controllers
{
    public class OrderController : Controller
    {
        ...
        {
            var response = await _orderService.UpdateOrderStatus(SD.Status_ReadyForPickup, orderId);
            if (response != null && response.isSuccess)
            {
        ...
        {
            var response = await _orderService.UpdateOrderStatus(SD.Status_Completed, orderId);
            if (response != null && response.isSuccess)
            {
                TempData["success"] = "Status Updated Successfully";
                return RedirectToAction(nameof(OrderDetail), new { orderId = orderId });
        ...
        {
            var response = await _orderService.UpdateOrderStatus(SD.Status_Cancelled, orderId);
            if (response != null && response.isSuccess)
            {
                TempData["success"] = "Status Updated Successfully";
                return RedirectToAction(nameof(OrderDetail), new { orderId = orderId });
